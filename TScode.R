...
####Determine the observed FST

setwd("/home/shehs/sparrow_accli_reseq/genic_region")
file<-list.files(pattern = "*.fst", recursive = F)
mean<-c()
col<-c()
for (i in 1:length(file)) { 
  fst<-read.table(file[i],header=T, sep= "\t")
  fst$WEIR_AND_COCKERHAM_FST[fst$WEIR_AND_COCKERHAM_FST<0]<-0
  mean[i]<-mean(fst$WEIR_AND_COCKERHAM_FST)
  col[i]<-file[i]
}
mean<-as.data.frame(mean)
col<-as.data.frame(col)
all<-cbind(col,mean)
write.table(all,"observed_fst.txt", row.names=F,quote=F,sep="\t")


####Determine the 95% percentile of permutated FST distribution

ls<-read.csv("file_list.txt",header=F,sep="\t")
for (i in 1:nrow(ls)){
  fi<- paste0("/home/shehs/sparrow_accli_reseq/genic_region/", as.character(ls[i,1]))
  setwd(fi)
  file<-list.files(pattern = "*.weir.fst", recursive = F)
  col<-c()
  mean<-c()
  for (j in 1:length(file)) { 
    fst<-read.table(file[j],header=T, sep= "\t")
    fst$WEIR_AND_COCKERHAM_FST[fst$WEIR_AND_COCKERHAM_FST<0]<-0
    col[j]<- file[j]
    mean[j]<-mean(fst$WEIR_AND_COCKERHAM_FST)
  }
  mean<-as.data.frame(mean)
  col<-as.data.frame(col)
  all<-cbind(col,mean)
  write.table(all,"mean.txt", row.names=F,quote=F,sep="\t")
}


ls<-read.csv("file_list.txt",header=F,sep="\t")
obj<-read.csv("observed_fst.txt",header=T,sep="\t")
CI<-c()
P<-c()
for (i in 1:nrow(ls)){
  fi<- paste0("/home/shehs/sparrow_accli_reseq/genic_region/", as.character(ls[i,1]))
  setwd(fi)
  data<-read.csv("mean.txt",header=T,sep="\t")
  ci<-data[order(data$mean),]
  CI[i]<-ci$mean[95]
  obji<-obj[i,2]
  P[i]<-sum(data$mean>obji)/100
}
all<-data.frame(CI=CI,P=P)
all1<-cbind(obj,all)
write.table(all1,"/home/shehs/sparrow_accli_reseq/genic_region/permutated_Pvalue.txt", row.names=F,quote=F,sep="\t")
